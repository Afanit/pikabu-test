...
class SingleScrollDirectionEnforcer private constructor() : RecyclerView.OnScrollListener(),
    ...
    override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
        ...
        if (oldState == RecyclerView.SCROLL_STATE_IDLE && newState == RecyclerView.SCROLL_STATE_DRAGGING) {
            ...
            val singleDirectionScrollable = canScrollHorizontally != canScrollVertically
            val preventHorizontalScroll = canScrollHorizontally && abs(dy) > abs(dx)
            val preventVerticalScroll = canScrollVertically && abs(dx) > abs(dy)

            if (singleDirectionScrollable && (preventHorizontalScroll || preventVerticalScroll)) {
